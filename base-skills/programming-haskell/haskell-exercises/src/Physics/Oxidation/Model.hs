module Physics.Oxidation.Model ( modelSolver ) where

import System.Directory (createDirectoryIfMissing)
import Physics.Typing (Nb)
import Physics.Oxidation.OdeSystem (rhs)
import Physics.Numerical.Integrator (euler)
...
modelSolver t tk tw l h tc tm yf dy = do
    ...
    let ode (_, t0, tk0) = rhs t0 tk0 tw l h tc tm
    let sol = euler ode (0, t, tk) dy yf

    createDirectoryIfMissing True "results"
    writeFile "results/Oxidation.csv" (toCsv sol ',')
    putStrLn "*** GOOD-BYE ***\n"  
