...
2. Specify Lennard-Jones interactions between type 0 particles with other type
   ...
   parameter to $2^{\frac16}\sigma$ to get de-mixing or to $2.5\sigma$ to get
...
with warnings.catch_warnings():
    ...
    for _ in range(no_part):
        ...
        system.part.add(type=0, pos=pos)
    ...
    plt.ylabel("$g(r)$")
    plt.tight_layout()
    plt.savefig("results/001-01-radial-function", dpi=300)
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig("results/001-01-temperature", dpi=300)
    ...
    plt.yscale("log")
    plt.legend()
    plt.savefig("results/001-01-particle-msd", dpi=300)
